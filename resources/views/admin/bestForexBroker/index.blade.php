                                ...
                                <tbody>
                                    @forelse ($categories as $key => $category)
                                        <tr>
                                            ...
                                            <td class="align-middle text-capitalize">{{ $category->use_menu }}</td>
                                            <td class="align-middle">
                                                <form action="{{ route('best-forex-broker-sub-category.destroy', $category->id) }}" method="POST" class="text-nowrap">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a href="{{ route('best-forex-broker-sub-category.edit', $category->id) }}" class="btn btn-info btn-sm mx-1" target="_blank">Edit</a>
